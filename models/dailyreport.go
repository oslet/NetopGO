...
func GetDailyReport() string {
	...
	defer db.Close()
	rows, err := db.Query("SELECT appname, appcontent, applicant, publisher, department, publishtime, followstatus, followman FROM dailyreport")
	...
	for rows.Next() {
		...
		if err == nil {
			...
			dailyreports = append(dailyreports, Dailyreport{appname, appcontent, applicant, publisher, department, publishtime, followstatus, followman})
		}
	}
